...
// Get settings
$query_settings = $database->query("SELECT * FROM `".TABLE_PREFIX."mod_faq_settings` WHERE section_id='$section_id' LIMIT 1");
$fetch_settings = $query_settings->fetchRow();

$header= $admin->strip_slashes($fetch_settings['header']);
$footer= $admin->strip_slashes($fetch_settings['footer']);
$template_summary = $admin->strip_slashes($fetch_settings['template_summary']);
...
if($query_cats->numRows() > 0)
{
	...
	{
		echo strtr($category_summary_first, array('{CATEGORY}' => $admin->strip_slashes($cat['cat_name'])));
...
if($query_cats->numRows() > 0)
{
	while($cat = $query_cats->fetchRow())
	{
		echo strtr($category_detail_first, array('{CATEGORY}' => $admin->strip_slashes($cat['cat_name'])));
		$query_quests = $database->query("SELECT * FROM `".TABLE_PREFIX."mod_faq_questions` WHERE cat_id='".$cat['cat_id']."' AND section_id='".$section_id."' ORDER BY pos ASC");
		if($query_quests->numRows() > 0)
		{
			while($quest = $query_quests->fetchRow())
			{
				...
				$replace_pattern = array('{QUESTION}' => $admin->strip_slashes($quest['question']), '{LINK}' => "question_".$quest['question_id'], '{ANSWER}' => $content);
