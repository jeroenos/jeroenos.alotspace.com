...
$query_question = $database->query("SELECT * FROM ".TABLE_PREFIX."mod_faq_questions WHERE question_id='$question_id' AND section_id='".$section_id."' ");
...
<form name="modify" action="<?php echo WB_URL; ?>/modules/faqbaker/save_question.php" method="post" style="margin: 0;">
...
<tr>
	<td class="setting_name">
		...
		<?php
		// Get categories
		$query_categories = $database->query("SELECT * FROM ".TABLE_PREFIX."mod_faq_categories WHERE section_id='".$section_id."' ORDER BY pos ASC");
...
if(!isset($wysiwyg_editor_loaded)) {
	...
	if (!defined('WYSIWYG_EDITOR') OR WYSIWYG_EDITOR=="none" OR !file_exists(WB_PATH.'/modules/'.WYSIWYG_EDITOR.'/include.php')) {
		...
		$id_list=array();
		$query_wysiwyg = $database->query("SELECT section_id FROM ".TABLE_PREFIX."sections WHERE page_id = '$page_id' AND module = 'faqbaker'");
