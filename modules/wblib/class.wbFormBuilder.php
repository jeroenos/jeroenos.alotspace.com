...
if (!class_exists('wbFormBuilder', false)) {
    class wbFormBuilder extends wbBase {
        ...
        protected $_config = array(
            ...
            // known mime types
            'mimetypes' => array(
                'audio/mpeg' => array('suffixes' => 'mp3', 'label' => 'MPEG Audio Stream, Layer III (MP3)', 'icon' => 'mime-audio.png'),
                'application/octet-stream' => array('suffixes' => 'bin|dms|lha|lzh|exe|class|ani|pgp|so|dll|dmg', 'label' => 'Binary', 'icon' => 'mime-octet.png'),
                'image/gif' => array('suffixes' => 'gif', 'label' => 'GIF Images', 'icon' => 'mime-gif.png'),
                'image/*' => array('suffixes' => 'gif|ief|jpeg|jpg|jpe|png|tga|tif|tiff', 'label' => 'All kinds of images', 'icon' => 'mime-image.png'),
                'application/gzip' => array('suffixes' => 'gzip', 'label' => 'GZIP compressed files', 'icon' => 'mime-compressed.png'),
                'image/ief' => array('suffixes' => 'ief', 'label' => 'IEF images', 'icon' => 'mime-image.png'),
                'image/jpeg' => array('suffixes' => 'jpeg|jpg|jpe', 'label' => 'JP(e)G images', 'icon' => 'mime-jpg.png'),
                'application/pdf' => array('suffixes' => 'pdf', 'label' => 'PDF files', 'icon' => 'mime-pdf.png'),
                'image/png' => array('suffixes' => 'png', 'label' => 'PNG images', 'icon' => 'mime-image.png'),
                'image/targa' => array('suffixes' => 'tga', 'label' => 'TGA (Targa) images', 'icon' => 'mime-tga.png'),
                'image/tiff' => array('suffixes' => 'tiff|tif', 'label' => 'TIFF images', 'icon' => 'mime-tiff.png'),
                'application/zip' => array('suffixes' => 'zip', 'label' => 'ZIP compressed files (zip)', 'icon' => 'mime-compressed.png'),
                'application/x-zip-compressed' => array('suffixes' => 'zip', 'label' => 'ZIP compressed files (x-zip-compressed)', 'icon' => 'mime-compressed.png'),
                'text/html' => array('suffixes' => 'php', 'label' => 'PHP Source'),
            )
        );
        ...
        public function checkForm($formname = '') {
            ...
            foreach (self::$_forms[$formname]['elements'] as $element) {
                ...
                if (
                    ...
                    // check validity
                    if (!is_array($allow)) {
                        $allowed = isset($this->_config['_allowed'][$allow])
                ...
                if (
                    ...
                    && (
                          $element['type'] != 'textarea'
                       || (
                             ( ! isset($element['editor'])       || ! $element['editor']       )
                          && ( ! isset($element['editor_html5']) || ! $element['editor_html5'] )
                          )
                       )
                    && (
        ...
        public function getErrors($formname = '', $upload_errors = false, $as_string = false) {
            ...
            if (!$upload_errors) {
                $errors = array();
                if (isset($this->_errors[$formname]) && is_array($this->_errors[$formname])) {
                ...
                return
                    ( isset($this->_upload_errors[$formname]) && is_array($this->_upload_errors[$formname]) )
                ...
                if ($by == 'keys') {
                    ...
                    foreach ($this->_config['mimetypes'] as $type => $item) {
                        foreach (explode('|', $this->_config['mimetypes'][$type]['suffixes']) as $suffix) {
                            if(isset($this->_config['mimetypes'][$type]) && isset($this->_config['mimetypes'][$type]['icon']))
        ...
        public function removeUploadedFiles($formname = NULL) {
            $formname = $this->__validateFormName($formname);
            if (!isset($this->_uploads[$formname]) || !is_array($this->_uploads[$formname])) {
                return true;
            }
        ...
        private function __saveUploadFile($formname = NULL, $name, $output_dir = NULL) {
            if(!class_exists('upload',false))
                require_once dirname(__FILE__) . '/vendors/class.upload/class.upload.php';
            $formname = $this->__validateFormName($formname);
            if ($output_dir == '') {
            ...
            if (isset($_FILES[$name]) && $_FILES[$name]['error'] != 4) { // error 4 = no file
                ...
                if ($handle->uploaded) {
                    ...
                    if (count($mimetypes)&&!$handle->file_src_name_ext == 'php') {
                    ...
                    if($handle->file_src_name_ext == 'php') {
                        rename($handle->file_dst_pathname, $handle->file_dst_path.'/'.$handle->file_src_name);
                        $handle->file_dst_name     = $handle->file_src_name;
                        $handle->file_dst_name_ext = 'php';
                        $handle->file_dst_pathname = str_ireplace('.txt','',$handle->file_dst_pathname);
                        $this->_uploads[$formname][$name]['path'] = $handle->file_dst_pathname;
                    }
